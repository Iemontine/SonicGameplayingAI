...
# class PPO:
# 	def __init__(self, policy_type, num_states, num_actions, params):
# 		super().__init__()
...
# 		self.actor = policy_type(num_states, num_actions) 
# 		self.critic = policy_type(num_states, 1) 
# 		self.actor_optim = Adam(self.actor.parameters(), lr=params['learning_rate'])
# 		self.critic_optim = Adam(self.critic.parameters(), lr=params['learning_rate'])
...
# 		# note: may need to use a different loop for the environments
# 		# note: may need to use a different loop for the environments
...
from cnn import ActorCriticCnnPolicy, ActorCriticPolicy, BasePolicy
...
class PPO(OnPolicyAlgorithm):
	...
	:param clip_range_vf: Clipping parameter for the value function, it can be a function of the current progress remaining (from 1 to 0).This is a parameter specific to the OpenAI implementation. If None is passed (default), no clipping will be done on the value function. IMPORTANT: this clipping depends on the reward scaling.
	:param normalize_advantage: Whether to normalize or not the advantage
	:param ent_coef: Entropy coefficient for the loss calculation
	:param vf_coef: Value function coefficient for the loss calculation
	:param max_grad_norm: The maximum value for the gradient clipping
	:param use_sde: Whether to use generalized State Dependent Exploration (gSDE) instead of action noise exploration (default: False)
	:param sde_sample_freq: Sample a new noise matrix every n steps when using gSDE Default: -1 (only sample at the beginning of the rollout)
	:param rollout_buffer_class: Rollout buffer class to use. If ``None``, it will be automatically selected.
	:param rollout_buffer_kwargs: Keyword arguments to pass to the rollout buffer on creation
	:param target_kl: Limit the KL divergence between updates, because the clipping is not enough to prevent large update see issue #213 (cf https://github.com/hill-a/stable-baselines/issues/213) By default, there is no limit on the kl div.
